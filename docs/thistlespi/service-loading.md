...
* 开源库或框架层开发者请阅读全部内容
* 用户工程或插件库开发者请阅读`用户工程或插件库(上层)实现服务` `用户指定服务`
* 单纯想了解指定服务/排除插件/解决冲突的方法请阅读`用户指定服务`
...
* 服务实例化时, 会根据服务定义文件中的构造参数的值, 作为配置文件名, 找到服务定义文件所在工程下的配置文件, 加载其中的配置, 最后将Properties传入这个构造器
* 注意这个构造参数可能为空(如果服务定义文件中未设置构造参数)
...
* 默认情况下, 程序会采用优先级最高的实现, 无需进行指定(`application`>`platform`>`library`>`default`)
